...
import { type Organization, organizationSchema } from './tables/organizations.js';

/** Where this table's shape is served. Derived so client and server cannot drift. */
export const organizationsShapePath = shapePathFor('organizations');

export function createOrganizationsCollection(options: SyncCollectionClientOptions) {
	...
	return createCollection(
		electricCollectionOptions({
			...syncCollectionConfig<Organization>({ table: 'organizations', ...options }),
