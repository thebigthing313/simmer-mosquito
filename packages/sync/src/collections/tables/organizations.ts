 ...
 * organization's own record. They are written and read in the operator
 * console (`apps/admin`), which reaches them over REST; `subscription_notes`
 * in particular is what operators write *about* an organization. An
 * organization that should see its own subscription state is a product
 * decision to make deliberately, not a column to leave streaming by default.
 * Say so in `WITHHELD` in `scripts/withheld-columns.mjs`, never by deleting
 * a line below: `pnpm check:schemas` refuses a field list that is not the
 * one that file generates, and the drift check reads the same list.
 *
