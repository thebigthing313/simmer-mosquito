...
import { type Formulation, formulationSchema } from './tables/formulations.js';

/** Where this table's shape is served. Derived so client and server cannot drift. */
export const formulationsShapePath = shapePathFor('formulations');

export function createFormulationsCollection(options: SyncCollectionClientOptions) {
	...
	return createCollection(
		electricCollectionOptions({
			...syncCollectionConfig<Formulation>({ table: 'formulations', ...options }),
