...
import { type User, userSchema } from './tables/users.js';

/** Where this table's shape is served. Derived so client and server cannot drift. */
export const usersShapePath = shapePathFor('users');

export function createUsersCollection(options: SyncCollectionClientOptions) {
	...
	return createCollection(
		electricCollectionOptions({
			...syncCollectionConfig<User>({ table: 'users', ...options }),
			schema: userSchema,
		}),
	);
}
