...
import { type Inspection, inspectionSchema } from './tables/inspections.js';

/** Where this table's shape is served. Derived so client and server cannot drift. */
export const inspectionsShapePath = shapePathFor('inspections');

export function createInspectionsCollection(options: SyncCollectionClientOptions) {
	...
	return createCollection(
		electricCollectionOptions({
			...syncCollectionConfig<Inspection>({ table: 'inspections', ...options }),
