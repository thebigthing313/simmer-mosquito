...
import { type Address, addressSchema } from './tables/addresses.js';

/** Where this table's shape is served. Derived so client and server cannot drift. */
export const addressesShapePath = shapePathFor('addresses');

export function createAddressesCollection(options: SyncCollectionClientOptions) {
	...
	return createCollection(
		electricCollectionOptions({
			...syncCollectionConfig<Address>({ table: 'addresses', ...options }),
