...
import { type CollectionMethod, collectionMethodSchema } from './tables/collection_methods.js';

/** Where this table's shape is served. Derived so client and server cannot drift. */
export const collectionMethodsShapePath = shapePathFor('collection_methods');

export function createCollectionMethodsCollection(options: SyncCollectionClientOptions) {
	...
	return createCollection(
		electricCollectionOptions({
			...syncCollectionConfig<CollectionMethod>({ table: 'collection_methods', ...options }),
