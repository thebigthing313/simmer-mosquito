...
import { type Unit, unitSchema } from './tables/units.js';

/** Where this table's shape is served. Derived so client and server cannot drift. */
export const unitsShapePath = shapePathFor('units');

export function createUnitsCollection(options: SyncCollectionClientOptions) {
	...
	return createCollection(
		electricCollectionOptions({
			...syncCollectionConfig<Unit>({ table: 'units', ...options }),
			schema: unitSchema,
		}),
	);
}
