...
import { type ApplicationMethod, applicationMethodSchema } from './tables/application_methods.js';

/** Where this table's shape is served. Derived so client and server cannot drift. */
export const applicationMethodsShapePath = shapePathFor('application_methods');

export function createApplicationMethodsCollection(options: SyncCollectionClientOptions) {
	...
	return createCollection(
		electricCollectionOptions({
			...syncCollectionConfig<ApplicationMethod>({ table: 'application_methods', ...options }),
			schema: applicationMethodSchema,
		}),
	);
}
