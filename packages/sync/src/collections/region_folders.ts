...
import { type RegionFolder, regionFolderSchema } from './tables/region_folders.js';
...
export function createRegionFoldersCollection(options: SyncCollectionClientOptions) {
	...
	return createCollection(
		electricCollectionOptions({
			...syncCollectionConfig<RegionFolder>({ table: 'region_folders', ...options }),
			schema: regionFolderSchema,
		}),
	);
}
