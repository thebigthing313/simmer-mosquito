...
import {
	...
	formulationInsecticideSchema,
} from './tables/formulation_insecticides.js';
 ...
 */
export type { FormulationInsecticide };

/** Where this table's shape is served. Derived so client and server cannot drift. */
export const formulationInsecticidesShapePath = shapePathFor('formulation_insecticides');

export function createFormulationInsecticidesCollection(options: SyncCollectionClientOptions) {
	...
	return createCollection(
		electricCollectionOptions({
			...syncCollectionConfig<FormulationInsecticide>({
				table: 'formulation_insecticides',
				...options,
			}),
			schema: formulationInsecticideSchema,
		}),
	);
}
