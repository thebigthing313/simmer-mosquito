...
import { type OutreachMethod, outreachMethodSchema } from './tables/outreach_methods.js';

/** Where this table's shape is served. Derived so client and server cannot drift. */
export const outreachMethodsShapePath = shapePathFor('outreach_methods');

export function createOutreachMethodsCollection(options: SyncCollectionClientOptions) {
	...
	return createCollection(
		electricCollectionOptions({
			...syncCollectionConfig<OutreachMethod>({ table: 'outreach_methods', ...options }),
			schema: outreachMethodSchema,
		}),
	);
}
