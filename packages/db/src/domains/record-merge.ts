 ...
 *
 * The confirmation the UI shows before the user commits, counted from the same
 * rules the write uses so the number they agreed to is the number that moves.
 * Reports zero-row rules as absent, the way `readDeleteImpact` does.
 */
export async function readMergeImpact(
	db: DbExecutor,
	input: MergeInput,
): Promise<readonly MergeMoveEntry[]> {
	const config = MERGEABLE_RECORDS[input.recordType];
	if (config.rules.length === 0 || input.sourceIds.length === 0) {
		return [];
	}

	const parts: RawBuilder<unknown>[] = config.rules.map(
		(rule) => sql`
			select ${rule.key}::text as key, count(*)::text as count
			from ${sql.table(rule.table)}
			where ${sourceMatch(rule.scope, input.sourceIds)}
				and organization_id = ${input.organizationId}
				and deleted_at is null
		`,
	);
