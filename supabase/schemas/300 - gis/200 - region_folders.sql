create table public.region_folders (
    ...
    constraint unique_region_folder_name unique (group_id, region_folder_name)
);

create trigger set_audit_fields
before insert or update on public.region_folders
for each row
execute function public.set_audit_fields();

create trigger soft_delete_trigger
before delete on public.region_folders
for each row
