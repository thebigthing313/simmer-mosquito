 ...
 *
 * `serverUrl` is the same call in all three of them, and the development-only
 * visibility adapter has to be the same in all three or the one file that missed
 ...
 * when that tab is fronted, so browser verification of the operator console hung on a loading
 ...
 * bug, run a build (`pnpm --filter @simmer-mosquito/admin build` and preview it)
