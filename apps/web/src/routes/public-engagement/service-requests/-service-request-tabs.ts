...
export const SERVICE_REQUEST_TABS = [
	...
	'control',
	'comments',
] as const satisfies readonly (NearbyFamily | 'details' | 'comments')[];
 ...
 * the request and its radius are the whole picture there. The nearby service
 * requests those two tabs will add are #1090's, and this is the one function
 * that decides what the map is handed per tab, so that change lands here.
 */
export function mapFamiliesForTab(tab: ServiceRequestTab): ReadonlySet<NearbyFamily> {
	const family = tabFamily(tab);
	return family === null ? new Set() : new Set([family]);
}
