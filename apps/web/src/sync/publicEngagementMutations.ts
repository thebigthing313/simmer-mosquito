...
/**
 * Public engagement optimistic mutation handlers: notification registrations
 * (+ registration-type subscriptions) and mission notification status
 * transitions.
 *
 * What is left after contacts and service requests moved to
 * `hooks/mutations` — both now name the command they mean rather than sending a
 * body for the server to infer one from. Registration-type subscriptions use the
 * shared {@link createRecordHandlers} factory; notification registrations
 * reference an existing contact/address and carry their point geometry through
 * `metadata.geometry`; mission notifications are generated server-side, so only
...
// ---------------------------------------------------------------------------
// Generic factory (registration-type subscriptions)
// ---------------------------------------------------------------------------
