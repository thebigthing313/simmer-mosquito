...
// The synced row carries the centroid (lat/lng) but not the geojson, which stays
...
/**
 * One address's drawable geometry, read over HTTP from `/map/addresses/:id`
 * and cached for the session. `null` when the address does not exist.
 */
