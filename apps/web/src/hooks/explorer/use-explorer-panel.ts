...
import { type MapInset, NO_MAP_INSET } from '../../components/map/map-inset';
import { useMeasuredBox } from './use-measured-box';
...
/**
 * The results panel's own state and the map inset it produces.
 *
 * The collapse flag is component state rather than a search param. The route
 * calls this rather than the explorer frame, because a route reads the inset
 * to place its own chrome over the same map.
 */
