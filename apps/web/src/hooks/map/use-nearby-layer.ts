 ...
 * marker, and the nearby records coloured by family.
 *
 * Takes one FeatureCollection whose features carry a `role` (`ring`, `center` or
 * `nearby`); nearby points also carry `family` and `id`. A family is toggled by
 * the caller omitting its features from `data`.
 */
