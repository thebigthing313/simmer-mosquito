			...
			query: (query) =>
				query
					...
					.select(({ sample, inspection, habitat }) => ({
						...
						//
						// There is no guard to write instead. The trap in `HANDOFF.md` says to
						// test the driving row's own column — but `sample.inspection_id` is not
						// nullable, so it cannot say whether the Inspection has arrived. Here
						// "unmatched" only ever means "still streaming".
