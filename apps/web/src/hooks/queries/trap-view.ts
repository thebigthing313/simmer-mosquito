 ...
 * falling back to a short id when neither is — `trapDisplayName` in
 * `routes/adult-surveillance/-adult-display.tsx`, which every surface already
 * shares. That last branch is what keeps the rule out of the query: the fallback
 * is a substring of the id, and the expression language has no substring. The
 * same reason `Sample.name` is nullable (`sample-view.ts`).
 *
 ...
 * comment gives. Pass one of these to `trapDisplayName`.
 */
