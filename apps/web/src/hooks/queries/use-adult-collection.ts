	...
	const result = useRecordById({
		...
		query: (query) =>
			query
				...
				.join(
					{ trap: traps() },
					({ record: collection, trap }) => eq(collection.trap_id, trap.id),
					'left',
				)
				.join(
					{ method: collection_methods() },
					({ record: collection, method }) => eq(collection.collection_method_id, method.id),
					'left',
				)
				.join(
					{ lure: collection_lures() },
					({ record: collection, lure }) => eq(collection.collection_lure_id, lure.id),
					'left',
				)
				.join(
					{ address: addresses() },
					({ record: collection, address }) => eq(collection.address_id, address.id),
					'left',
				)
