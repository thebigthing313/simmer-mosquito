...
import { type BiocontrolMethod, createBiocontrolMethodsCollection } from '@simmer-mosquito/sync';
 ...
 *
 * This app writes biocontrol_methods, so the collection carries the three
 ...
 */
export const biocontrol_methods: Collection<BiocontrolMethod, string | number> =
	createBiocontrolMethodsCollection({
		...syncClientOptions,
		syncMode: 'eager',
		mutations: true,
	});
 ...
 */
biocontrol_methods.createIndex((row) => row.id, { indexType: BasicIndex });
