/**
 * The `assignment_items` collection.
 *
 * How the table streams and whether this app may write to it are the app's
 * decisions rather than the table's — see `SyncCollectionClientOptions` in
 * `packages/sync` for why a schema cannot answer either.
 */

import { type AssignmentItem, createAssignmentItemsCollection } from '@simmer-mosquito/sync';
import { declareCollection } from './registry';

/**
 * `on-demand`: One row per stop on an assignment, and only the run on screen needs its own.
 *
 * This app writes assignment_items, so the collection carries the three
 * mutation handlers and every write through it names the command it means.
 */
export const assignment_items = declareCollection<AssignmentItem>({
	table: 'assignment_items',
	syncMode: 'on-demand',
	mutations: true,
	create: createAssignmentItemsCollection,
});
