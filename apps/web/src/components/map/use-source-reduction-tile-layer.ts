...
const selectedLayerIds = SOURCE_REDUCTION_SELECTED_LAYER_IDS as readonly string[];

/**
 * Binds the source-reduction vector-tile source + layers to a live Mapbox map.
 * Survives basemap restyles (re-adds on `style.load`), pushes filter changes
 * through `setTiles` without re-adding layers, keeps the selection highlight in
 * sync, and routes clicks/hover on activity features back out through
 * `onSelectFeature`. A no-op when `config` is undefined. Mirrors
 * {@link useChemicalTileLayer}.
 */
export function useSourceReductionTileLayer(
	...
	const url = enabled ? buildSourceReductionTileUrl(config.serverUrl, config.filters) : null;
	...
	useEffect(() => {
		...
		function ensureLayers() {
			...
			if (activeMap.getSource(SOURCE_REDUCTION_SOURCE_ID) === undefined) {
				activeMap.addSource(SOURCE_REDUCTION_SOURCE_ID, {
					type: 'vector',
					tiles: [currentUrl],
					promoteId: 'id',
				});
			}
			for (const layer of sourceReductionTileLayers(selectedRef.current)) {
		...
		function presentInteractiveLayers(): string[] {
			return SOURCE_REDUCTION_INTERACTIVE_LAYER_IDS.filter(
				(id) => activeMap.getLayer(id) !== undefined,
			);
		}
		...
		return () => {
			...
			try {
				...
				for (const id of SOURCE_REDUCTION_LAYER_IDS) {
					if (activeMap.getLayer(id) !== undefined) {
						activeMap.removeLayer(id);
					}
				}
				if (activeMap.getSource(SOURCE_REDUCTION_SOURCE_ID) !== undefined) {
					activeMap.removeSource(SOURCE_REDUCTION_SOURCE_ID);
				}
			} catch {
	...
	useEffect(() => {
		...
		const source = map.getSource(SOURCE_REDUCTION_SOURCE_ID) as VectorTileSource | undefined;
		source?.setTiles?.([url]);
	}, [map, isLoaded, url]);

	// Re-scope the highlight layers to the selected feature. Guarded by `enabled`
	// so maps without a source-reduction config never touch layers that were never added.
	useEffect(() => {
		if (map === null || !isLoaded || !enabled) {
			return;
		}
		try {
			for (const layer of sourceReductionTileLayers(selectedId)) {
