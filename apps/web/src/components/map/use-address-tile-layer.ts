...
export interface AddressTileLayerConfig {
	/** Base server URL the tile template is built against. */
	readonly serverUrl: string;
	/** Server-side filters folded into the tile request. */
	readonly filters?: AddressTileFilters;
	/** Currently selected address id; drives the on-map highlight. */
	readonly selectedId?: string | null;
	/** Fired with an address id on feature click, or null when clicking empty map. */
	readonly onSelectFeature?: (id: string | null) => void;
}

const selectedLayerIds = ADDRESS_SELECTED_LAYER_IDS as readonly string[];

/**
 * Binds the address vector-tile source + layers to a live Mapbox map. Survives
 ...
 * routes clicks/hover on address points back out through `onSelectFeature`.
 * A no-op when `config` is undefined. Mirrors {@link useTrapTileLayer}.
 */
export function useAddressTileLayer(
	...
	const url = enabled ? buildAddressTileUrl(config.serverUrl, config.filters) : null;
	...
	useEffect(() => {
		...
		function ensureLayers() {
			...
			if (activeMap.getSource(ADDRESS_SOURCE_ID) === undefined) {
				activeMap.addSource(ADDRESS_SOURCE_ID, {
					type: 'vector',
					tiles: [currentUrl],
					promoteId: 'id',
				});
			}
			for (const layer of addressTileLayers(selectedRef.current)) {
		...
		function presentInteractiveLayers(): string[] {
			return ADDRESS_INTERACTIVE_LAYER_IDS.filter((id) => activeMap.getLayer(id) !== undefined);
		}
		...
		return () => {
			...
			try {
				...
				for (const id of ADDRESS_LAYER_IDS) {
					if (activeMap.getLayer(id) !== undefined) {
						activeMap.removeLayer(id);
					}
				}
				if (activeMap.getSource(ADDRESS_SOURCE_ID) !== undefined) {
					activeMap.removeSource(ADDRESS_SOURCE_ID);
				}
			} catch {
	...
	useEffect(() => {
		...
		const source = map.getSource(ADDRESS_SOURCE_ID) as VectorTileSource | undefined;
		source?.setTiles?.([url]);
	}, [map, isLoaded, url]);

	// Re-scope the highlight layer to the selected feature. Guarded by `enabled`
	// so maps without an address config never touch layers that were never added.
	useEffect(() => {
		if (map === null || !isLoaded || !enabled) {
			return;
		}
		try {
			for (const layer of addressTileLayers(selectedId)) {
