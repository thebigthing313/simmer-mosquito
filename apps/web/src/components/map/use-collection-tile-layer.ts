...
export interface CollectionTileLayerConfig {
	/** Base server URL the tile template is built against. */
	readonly serverUrl: string;
	/** Server-side filters folded into the tile request. */
	readonly filters?: CollectionTileFilters;
	/** Currently selected collection id; drives the on-map highlight. */
	readonly selectedId?: string | null;
	/** Fired with a collection id on feature click, or null when clicking empty map. */
	readonly onSelectFeature?: (id: string | null) => void;
}

const selectedLayerIds = COLLECTION_SELECTED_LAYER_IDS as readonly string[];

/**
 * Binds the collection vector-tile source + layers to a live Mapbox map. Survives
 ...
 * routes clicks/hover on collection features back out through `onSelectFeature`.
...
export function useCollectionTileLayer(
	...
	const url = enabled ? buildCollectionTileUrl(config.serverUrl, config.filters) : null;
	...
	useEffect(() => {
		...
		function ensureLayers() {
			...
			if (activeMap.getSource(COLLECTION_SOURCE_ID) === undefined) {
				activeMap.addSource(COLLECTION_SOURCE_ID, {
					type: 'vector',
					tiles: [currentUrl],
					promoteId: 'id',
				});
			}
			for (const layer of collectionTileLayers(selectedRef.current)) {
		...
		function presentInteractiveLayers(): string[] {
			return COLLECTION_INTERACTIVE_LAYER_IDS.filter((id) => activeMap.getLayer(id) !== undefined);
		}
		...
		return () => {
			...
			try {
				...
				for (const id of COLLECTION_LAYER_IDS) {
					if (activeMap.getLayer(id) !== undefined) {
						activeMap.removeLayer(id);
					}
				}
				if (activeMap.getSource(COLLECTION_SOURCE_ID) !== undefined) {
					activeMap.removeSource(COLLECTION_SOURCE_ID);
				}
			} catch {
	...
	useEffect(() => {
		...
		const source = map.getSource(COLLECTION_SOURCE_ID) as VectorTileSource | undefined;
		source?.setTiles?.([url]);
	}, [map, isLoaded, url]);

	// Re-scope the highlight layers to the selected feature. Guarded by `enabled`
	// so maps without a collection config never touch layers that were never added.
	useEffect(() => {
		if (map === null || !isLoaded || !enabled) {
			return;
		}
		try {
			for (const layer of collectionTileLayers(selectedId)) {
