...
export interface ChemicalTileLayerConfig {
	/** Base server URL the tile template is built against. */
	readonly serverUrl: string;
	/** Server-side filters folded into the tile request. */
	readonly filters?: ChemicalTileFilters;
	/** Currently selected application id; drives the on-map highlight. */
	readonly selectedId?: string | null;
	/** Fired with an application id on feature click, or null when clicking empty map. */
	readonly onSelectFeature?: (id: string | null) => void;
}

const selectedLayerIds = CHEMICAL_SELECTED_LAYER_IDS as readonly string[];

/**
 * Binds the chemical vector-tile source + layers to a live Mapbox map. Survives
 ...
 * routes clicks/hover on application features back out through `onSelectFeature`.
 * A no-op when `config` is undefined. Mirrors {@link useInspectionTileLayer}.
 */
export function useChemicalTileLayer(
	...
	const url = enabled ? buildChemicalTileUrl(config.serverUrl, config.filters) : null;
	...
	useEffect(() => {
		...
		function ensureLayers() {
			...
			if (activeMap.getSource(CHEMICAL_SOURCE_ID) === undefined) {
				activeMap.addSource(CHEMICAL_SOURCE_ID, {
					type: 'vector',
					tiles: [currentUrl],
					promoteId: 'id',
				});
			}
			for (const layer of chemicalTileLayers(selectedRef.current)) {
		...
		function presentInteractiveLayers(): string[] {
			return CHEMICAL_INTERACTIVE_LAYER_IDS.filter((id) => activeMap.getLayer(id) !== undefined);
		}
		...
		return () => {
			...
			try {
				...
				for (const id of CHEMICAL_LAYER_IDS) {
					if (activeMap.getLayer(id) !== undefined) {
						activeMap.removeLayer(id);
					}
				}
				if (activeMap.getSource(CHEMICAL_SOURCE_ID) !== undefined) {
					activeMap.removeSource(CHEMICAL_SOURCE_ID);
				}
			} catch {
	...
	useEffect(() => {
		...
		const source = map.getSource(CHEMICAL_SOURCE_ID) as VectorTileSource | undefined;
		source?.setTiles?.([url]);
	}, [map, isLoaded, url]);

	// Re-scope the highlight layers to the selected feature. Guarded by `enabled`
	// so maps without a chemical config never touch layers that were never added.
	useEffect(() => {
		if (map === null || !isLoaded || !enabled) {
			return;
		}
		try {
			for (const layer of chemicalTileLayers(selectedId)) {
