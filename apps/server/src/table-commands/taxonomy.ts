 ...
 * `/admin/genera` and `/admin/species` call `createGenusWithTxid` and its
 * siblings directly, with no command, no permission map and no actor.
 *
 * The update helpers cannot be reused as they stand, and the reason is worth
 * recording: `updateGenus` and `updateSpecies` `.set()` every column
 * unconditionally, so they are whole-row replacements wearing an update's name.
 * `updateSpecies` in particular writes `genus_id: input.genusId ?? null` — a
