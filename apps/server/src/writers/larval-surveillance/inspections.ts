...
/**
 * What the `inspections` table can be asked to do.
 *
 * The assignment-execution command is a `fieldWork.*` command handled here
 * rather than a larval one, because the row being written is an inspection. The
 * command vocabulary follows the *unit of work* (a stop, closed by a record);
...
/**
 * The six inspection commands, in one writer.
 *
 * `table-commands/inspections.ts` is the only caller: it builds the command a
 * `/commands/inspections` body names and hands it here.
 */
