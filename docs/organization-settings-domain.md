...
4. **A typed calendar day widened into a `timestamptz` takes the Agency zone.**
   ...
   A form that also reads the stored instant back, such as a due time or a
   scheduled start, must read it with `localTimeOfDay` and the same zone, or an untouched
