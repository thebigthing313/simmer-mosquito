...
- **`CONTEXT.md`** at the repo root. It is the domain glossary: Agency, Profile,
  Membership, and the rest of the vocabulary the code is written in.
- **`docs/adr/`** at the repo root. Read the ADRs that touch the area you are
  about to work in, before you change it. Auth, sync, identity, tenancy, DB
  layering, and field-work provenance each have one.
...
drift to a synonym the glossary avoids. Agency, not tenant. Profile, not user.
Membership, not account.
